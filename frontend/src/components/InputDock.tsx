...
export default function InputDock({ state, errorMessage, onSend, onContinue, onRetry }: Props) {
      ...
      {isGenerating ? (
        ...
        <>
          ...
          <div className="flex gap-2 items-end">
            ...
            <button
              type="button"
              className="bg-[var(--color-accent-dim)] text-[var(--color-text)] rounded-md py-2.5 px-5 text-[13px] cursor-pointer font-medium tracking-wide hover:bg-[var(--color-accent)] transition-colors whitespace-nowrap h-[44px] disabled:cursor-not-allowed disabled:opacity-45"
              onClick={submit}
              disabled={!canSend}
            >
              发送
          ...
          <div className="flex justify-center pt-1 pb-0.5">
            <button
              type="button"
              className="bg-transparent border border-[var(--color-border)] text-[var(--color-text-2)] rounded-md py-1.5 px-5 text-xs cursor-pointer hover:border-[rgba(124,138,255,0.2)] hover:text-[var(--color-accent)] transition-all disabled:cursor-not-allowed disabled:opacity-45"
              onClick={onContinue}
          ...
          {state === 'error' && (
            <div className="flex items-center gap-2.5 py-2.5 px-3.5 mt-2 bg-[rgba(224,85,85,0.04)] border border-[rgba(224,85,85,0.12)] rounded-md text-[var(--color-err)] text-xs">
              <span className="min-w-0 truncate">生成失败：{errorMessage || '未知错误'}</span>
              <button
                type="button"
                className="ml-auto bg-transparent border border-[rgba(224,85,85,0.2)] text-[var(--color-err)] rounded px-2.5 py-1 text-[11px] cursor-pointer hover:bg-[rgba(224,85,85,0.08)] transition-colors"
                onClick={onRetry}
              >
                重试
              </button>
