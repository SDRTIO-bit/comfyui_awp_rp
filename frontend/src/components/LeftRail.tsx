  ...
  return (
    <nav className="w-11 min-w-11 flex flex-col items-center pt-1.5 gap-0.5 border-r border-[var(--color-border)] bg-[var(--color-bg-app)] z-10">
      {ORDER.map(r => (
        <button
          key={r}
          className={`group w-[34px] h-[34px] flex items-center justify-center rounded-md cursor-pointer border-none bg-transparent relative transition-all ${activeResource === r ? 'text-[var(--color-accent)] bg-[var(--color-accent-soft)]' : 'text-[var(--color-text-3)] hover:text-[var(--color-text-2)] hover:bg-[var(--color-bg-hover)]'}`}
          onClick={() => onToggle(r)}
          title={LABELS[r]}
          aria-label={LABELS[r]}
        >
          {ICONS[r]}
          <span className="absolute left-[42px] top-1/2 -translate-y-1/2 bg-[var(--color-bg-raised)] text-[var(--color-text-2)] text-[11px] py-1 px-2 rounded whitespace-nowrap pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity border border-[var(--color-border)] z-30">
            {LABELS[r]}
          </span>
        </button>
      ))}
