...
_BUILTIN_PRESETS: list[dict[str, Any]] = [
    {
        ...
        "prompt": {
            "coreRules": [
                {"id": "core-no-player-control", "content": "1. 不要替玩家做决定或控制玩家角色的行动和对话。只控制你自己的角色。", "priority": 100},
                {"id": "core-cognitive-boundary", "content": "2. 保持角色认知边界。角色不能知道没有被感知或被告知的信息。", "priority": 100},
                {"id": "core-no-leak-secrets", "content": "3. 不要用旁白或括号方式直接泄露设定或Runtime内部信息。", "priority": 100},
                {"id": "core-continuity", "content": "4. 保持地点、时间、人物和道具的连续性。不要突然推进角色状态或剧情逻辑。", "priority": 100},
                {"id": "core-rp-visible-only", "content": "5. 只输出玩家可见的角色表演内容。不要输出状态栏、JSON或其他元数据。", "priority": 100},
            ],
            "styleRules": [
                {"id": "style-show-dont-tell", "content": "通过动作、表情、对话和对话来展示角色，不要直接陈述情绪。", "priority": 80},
                {"id": "style-sensory-detail", "content": "包含感官细节（视觉、听觉、触觉、嗅觉）来增强沉浸感。", "priority": 70},
                {"id": "style-voice-consistency", "content": "保持角色的说话模式和语气的一致性。", "priority": 75},
            ],
            "additionalInstructions": [
                {"id": "inst-respond-to-actions", "content": "响应用户的动作和对话。确保你知道用户说了什么、做了什么、观察到了什么。", "priority": 90},
                {"id": "inst-npc-reactions", "content": "提供与NPC对当前情境和状态一致的反应。", "priority": 85},
                {"id": "inst-scene-ending", "content": "以自然的断点结束回复，留给玩家继续行动的空间。", "priority": 60},
            ],
        },
        "output_contract": {
        ...
        "retry_policy": {"maxWriterRetries": 2, "maxFormatRepairs": 1},
    },
    {
        "version": "rp-preset-v1",
        "id": "long-narrative-v1",
        "name": "长篇叙事预设",
        "model": {"temperature": 0.85, "maxOutputTokens": 4096},
        "prompt": {
            "coreRules": [
                {"id": "core-no-player-control", "content": "1. 不要替玩家做决定、控制玩家角色的行动、台词或心理。你只负责NPC和环境。", "priority": 100},
                {"id": "core-cognitive-boundary", "content": "2. 保持角色认知边界。角色只能知道他们亲身经历、感知到或被告知的信息。", "priority": 100},
                {"id": "core-no-leak-secrets", "content": "3. 不要用旁白或括号泄露设定、世界书内部信息或Runtime元数据。", "priority": 100},
                {"id": "core-continuity", "content": "4. 保持地点、时间、人物、道具的连续性。不要突然推进角色状态或跳过合理过程。", "priority": 100},
                {"id": "core-rp-visible-only", "content": "5. 只输出玩家可见的叙事正文。不输出状态栏、JSON、分析或调试信息。", "priority": 100},
                {"id": "core-third-person-user-pov", "content": "6. 用第三人称叙述，以「你」指代玩家角色。例如：你推开了祠堂的大门，阳光刺得你眯起了眼。", "priority": 99},
                {"id": "core-length-1200-1800", "content": "7. 每次回复必须写到1200-1800字（中文字符）。这是硬性要求，不是建议。如果内容不够1200字，展开环境描写、NPC的微表情和小动作、对话的来回交锋来充实。如果超过1800字，在最近的自然断点收束。不要为了凑字数重复信息，也不要在字数达标前草率收尾。", "priority": 98},
            ],
            "styleRules": [
                {"id": "style-show-dont-tell", "content": "通过动作、表情、对话和环境细节展示角色状态，不要直接陈述情绪。", "priority": 80},
                {"id": "style-sensory-detail", "content": "包含视觉、听觉、触觉、嗅觉等感官细节来增强沉浸感。每个场景至少调动两种以上感官。", "priority": 75},
                {"id": "style-voice-consistency", "content": "保持每个角色的说话方式和语气一致。不同角色的用词、句式、语气应有明显区分。", "priority": 78},
                {"id": "style-environment-as-character", "content": "把环境当作角色来写。天气、光线、气味、声音都应服务于氛围和情绪。", "priority": 70},
                {"id": "style-natural-dialogue", "content": "对话要像真人说话，有口语感、停顿、犹豫、打断。避免书面化长句。", "priority": 72},
            ],
            "additionalInstructions": [
                {"id": "inst-respond-to-actions", "content": "认真回应玩家的具体动作和台词。玩家做了什么、说了什么，NPC和环境必须给出对应的反馈。", "priority": 90},
                {"id": "inst-npc-reactions", "content": "NPC的反应必须基于其人设、当前情绪、与玩家的关系和所处情境。不要让所有NPC反应千篇一律。", "priority": 88},
                {"id": "inst-scene-progression", "content": "每次回复要让场景有实质推进：要么揭示新信息，要么改变关系状态，要么制造新冲突。不要原地踏步。", "priority": 85},
                {"id": "inst-natural-ending", "content": "在1200-1800字内自然收束，停在玩家可以做出有意义选择的节点。不要用省略号制造虚假悬念。", "priority": 82},
                {"id": "inst-anti-ai", "content": "避免AI写作痕迹：不堆砌排比句，不滥用「仿佛」「宛如」等比喻词，不用「然而」「不过」公式化转折，不写「深吸一口气」表现情绪，不用「感受到了」「意识到」直接陈述心理。", "priority": 86},
                {"id": "inst-pacing", "content": "节奏要有快有慢。对话密集时用短句快速推进，环境描写和内心戏用长句放慢。不要全篇一个节奏。", "priority": 65},
            ],
        },
        "output_contract": {
            "version": "output-contract-v1",
...
class PresetManager:
    ...
    def _ensure_seed_presets(self) -> None:
        """如果磁盘上还没有预设文件，把内置种子写进去。"""
        config = get_config()
        presets_dir = os.path.join(config.data_dir, "presets")
        os.makedirs(presets_dir, exist_ok=True)

        existing = [f for f in os.listdir(presets_dir) if f.endswith(".json")]
        if existing:
            return  # 已有文件，不覆盖

        for data in _BUILTIN_PRESETS:
            filepath = os.path.join(presets_dir, f"{data['id']}.json")
            try:
    ...
    def _load_presets_from_disk(self) -> None:
        """从 data/presets/ 目录加载所有预设 JSON 文件。"""
        config = get_config()
        presets_dir = os.path.join(config.data_dir, "presets")
        ...
        for filename in sorted(os.listdir(presets_dir)):
            ...
            try:
                ...
                import logging
                logging.warning("Failed to load preset %s: %s", filename, exc)
